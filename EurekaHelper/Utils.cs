...
namespace EurekaHelper
{
    ...
    internal class Utils
    {
        ...
        public static void GetFatePositionFromLgb(ushort territoryId, List<EurekaFate> fates)
        {
            ...
            {
                var fateSheetData = DalamudApi
                    .DataManager.GetExcelSheet<Fate>()!
                    .GetRow(fate.FateId);
                ...
                var vector = MapUtil.WorldToMap(
                    new Vector2(match.Transform.Translation.X, match.Transform.Translation.Z),
                    territoryType.Value.Map.Value
                );
        ...
        public static SeString MapLink(uint territoryId, uint mapId, Vector2 position)
        {
            ...
            var name =
                $"{mapPayload.PlaceName} ({position.X.ToString("00.0", CultureInfo.InvariantCulture)}, {position.Y.ToString("00.0", CultureInfo.InvariantCulture)})";
        ...
        public static SeString ItemLink(
            uint itemId,
            bool isHq = false,
        ...
        public static SeStringBuilder ArisuStringbuilder(
            string nextNmString,
        ...
        )
        {
            ...
            {
                var currTimeDiff =
                    time1 + TimeSpan.FromMilliseconds(EorzeaTime.EIGHT_HOURS) - DateTime.Now;
        ...
        )
        {
            ...
            if (instance != null)
            {
                ...
                if (drawCircle)
                {
                    ...
                    instance->AddGatheringTempMarker(
                        mapPayload.RawX / 1000,
                        mapPayload.RawY / 1000,
                        Constants.MapCircleRadius
                    );
                }

                instance->SetFlagMapMarker(
                    mapPayload.Map.Value.TerritoryType.Value.RowId,
                    mapPayload.Map.RowId,
                    mapPayload.RawX / 1000f,
                    mapPayload.RawY / 1000f
                );
                ...
                if (openMap)
                    instance->OpenMap(
                        ...
                        mapPayload.Map.Value.TerritoryType.Value.RowId,
                        type: drawCircle
                            ? FFXIVClientStructs.FFXIV.Client.UI.Agent.MapType.GatheringLog
                            : FFXIVClientStructs.FFXIV.Client.UI.Agent.MapType.FlagMarker
                    );
            }
        }

        public static void SetFlagMarker(
            EurekaFate fateInfo,
            bool openMap = false,
            bool randomizeCoords = false,
            bool drawCircle = false
            ...
            SetFlagMarker(
                fateInfo.TerritoryId,
                fateInfo.MapId,
                new Vector2(fateInfo.FatePosition.X, fateInfo.FatePosition.Y),
                openMap,
                randomizeCoords,
                drawCircle
            );
        ...
        public static float GetRandomizeFloat(float centerValue) =>
            (float)(
                centerValue
                + (random.NextDouble() * Constants.RandomizeRange * 2.0f - Constants.RandomizeRange)
            );
        ...
        public static int DatacenterToEurekaDatacenterId(string datacenterName) =>
            Constants.DatacenterStringToEurekaId.GetValueOrDefault(datacenterName, 0);
        ...
        public static bool IsWithinMinimumDistance(
            Vector3 position,
        ...
        {
            return DalamudApi
                .DataManager.Excel.GetSheet<Item>()!
                .GetRow(itemId)!
        ...
        public static string GetJobCategory(uint itemId)
        {
            return DalamudApi
                .DataManager.Excel.GetSheet<Item>()!
                .GetRow(itemId)!
                .ClassJobCategory.Value.Name.ToDalamudString()
                .ToString();
        }
        ...
        public static bool EnumSelector<T>(string label, string? tooltipMessage, ref T enumValue)
            where T : Enum
        {
            var saved = false;
