...
namespace EurekaHelper.System
{
    public class ElementalManager : IDisposable
    {
        ...
        {
            if (
                !EurekaHelper.Config.ElementalCrowdsource
                && !EurekaHelper.Config.DisplayElemental
                && !EurekaHelper.Config.DisplayElementalToast
            )
                return;

            var elementals = DalamudApi.ObjectTable.Where(x =>
                x is IBattleNpc bnpc && Constants.EurekaElementals.Contains(bnpc.NameId)
            );
            foreach (var elemental in elementals)
            {
                ...
                var eurekaElemental = new EurekaElemental(
                    elemental.Name.TextValue,
                ...
                if (EurekaHelper.Config.ElementalCrowdsource)
                {
                    ...
                    if (
                        !knownLocations.Any(x =>
                            Utils.IsWithinMinimumDistance(x, eurekaElemental.RawPosition, 15.0f)
                        )
                    )
                    {
                        EurekaHelper.PrintMessage(
                            "Elemental found that is not in the plugin database."
                        );
                        EurekaHelper.PrintMessage(
                            "Please send the following information to the developer on GitHub or Discord DM. You can find the contact information in the \"About\" tab."
                        );
                        EurekaHelper.PrintMessage(
                            "You can also opt-out of crowdsourcing for Elemental positions in the \"Elementals\" tab."
                        );
                        EurekaHelper.PrintMessage(
                            $"Send -> T: {DalamudApi.ClientState.TerritoryType} X: {eurekaElemental.RawPosition.X} Y: {eurekaElemental.RawPosition.Y} Z: {eurekaElemental.RawPosition.Z}"
                        );
                    }
                }
                ...
                if (EurekaHelper.Config.DisplayElemental)
                {
                    ...
                    if (EurekaHelper.Config.ElementalPayloadOptions != PayloadOptions.Nothing)
                    {
                        DalamudLinkPayload payload = DalamudApi.ChatGui.AddChatLinkHandler(
                            eurekaElemental.ObjectId,
                            (i, m) =>
                            {
                                Utils.SetFlagMarker(
                                    eurekaElemental.TerritoryId,
                                    eurekaElemental.MapId,
